...
// import { MdLockClock, MdArrowDropDownCircle, MdOutlineArrowDropDownCircle } from 'react-icons/md';
...
//     //     if (number < 36 && number > 0) setNumber(value);
//     //     else alert("Enter a valid number between 1 to 36");
//     // }
...
//         e.preventDefault();
//         if (number > 0 && number < 37 && betAmount > 99 && (selectColor === 'Red' || selectColor === 'Blue' || selectColor === 'Green' || selectColor === 'Yellow' || selectColor === 'Purple')) {
...
//             const response = await axios.post("/games/colorBallLottery/bets", {
//                 userID: userID,
//                 betNumber: number,
...
//                         const betsResponse = await axios.get(`/games/colorBallLottery/bets/${userID}`);
...
//                 setBetCount(0);
//                 setMaxBet(200); // Reset Max Bet
//             } else if (cooldown !== 0) {
//                 setWalletBalance((prevBal) => (prevBal + totalRewardAmount));
...
//         };
//     }, [countdownTimer, cooldown, userID, gameHistoryTable, number, selectColor, betAmount, counter, totalAmountBet, myBetsTable]);



//     return (
//         <div className="px-10 pb-10 flex flex-col bg-blue-900 items-center">
//             <div className="text-2xl md:text-4xl lg:text-5xl font-bold text-center mt-28 md:mt-32 text-white underline underline-offset-8 italic">Color Ball Game</div>
...
//                         <BiUserCircle size={30} className='mr-2' />{user.userID}
//                     </div>
//                     <div className="p-2 rounded-lg font-bold w-40 md:w-auto h-auto flex text-center justify-center align-items-center items-center">
//                         <BiSolidWalletAlt size={30} className='mr-2' /> {user.walletBalance}
//                     </div>
//                     <div className="p-2 rounded-lg font-bold w-40 md:w-auto h-auto flex flex-col text-center justify-center align-items-center items-center">
//                         <div>Reward</div>
//                         <div>Rs.{user.walletBalance}</div>
//                     </div>
//                 </div>
//             </nav>


//             <div className="text-center font-semibold bg-black text-white border-4 rounded-xl text-lg md:text-xl p-4 my-4 mt-8 flex flex-row items-center">Place Your Bet Below <MdArrowDropDownCircle size={30} className='ml-4' /></div>
...
//                         </div>
//                     </div>
//                     <div className='w-5/6 md:w-1/2 h-auto mx-auto my-4'>
//                         <div onClick={handleSubmitBet} className={`bg-green-900 p-2 rounded-lg h-auto flex flex-col mx-auto hover:scale-125 border-4 hover:bg-blue-500 ${betCount < 1 ? '' : 'hidden'}`}>
//                             Place Bet
//                         </div>
//                         <div onClick={handleSubmitBet} className={`bg-green-900 p-2 rounded-lg h-auto flex flex-col mx-auto hover:scale-125 border-4 hover:bg-blue-500 ${betCount >= 1 && betCount < 108 ? '' : 'hidden'}`}>
//                             Place More Bets
//                         </div>
//                     </div>
//                 </div>
...
//             <div className="text-center font-semibold bg-black text-white border-4 rounded-lg text-lg md:text-xl p-4 my-4 flex flex-row">Games History
//                 {/* <MdOutlineArrowDropDownCircle size={35} className='ml-4' />              */}
//             </div>
//             <div className="w-full lg:w-auto h-auto overflow-x-auto rounded-lg text-white items-center">
...
//                             <tr key={index} className={`${row.rewardAmount === 0 ? 'bg-white' : 'bg-green-500'}`}>
...
//             <button onClick={navigateToHome} className="fixed text-red-500 rounded-lg bg-white top-2 right-4 z-10 ">
