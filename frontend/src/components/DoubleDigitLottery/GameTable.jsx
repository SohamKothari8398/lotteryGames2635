...
function GameTable() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const gamesResponse = await service.get('/games/doubleDigitLottery/games');
                    ...
                    <tbody className='font-bold text-xs md:text-sm'>
                        {gamesTable.slice().reverse().map((row, index) => (
                            <tr key={index} className="bg-white">
                                <td className='text-center border-2 p-1'>{gamesTable.length - index}</td>
                                <td className='text-center border-2 p-1'>{row.gameID}</td>
                                <td className='text-center border-2 p-1'>{format(new Date(row.gameID * 1000), 'dd/MM/yyyy')}</td>
                                <td className='text-center border-2 p-1'>{format(new Date(row.gameID * 1000), 'HH:mm:ss')}</td>
                                <td className='text-center border-2 p-1'>{format(new Date(row.endTime * 1000), 'HH:mm:ss')}</td>
                                <td className='text-center border-2 p-1'>{row.totalBets}</td>
                                <td className='text-center border-2 p-1'>{row.totalAmount}</td>
                                <td className='text-center border-2 p-1 text-green-700 text-base hover:scale-125 cursor-pointer'>
                                    {row.winningNumber}
                                </td>
                            </tr>
                        ))}
